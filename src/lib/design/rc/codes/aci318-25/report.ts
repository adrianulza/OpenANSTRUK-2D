 ...
 * Advanced Report deck (Design tab, "As checked" mode). ACI 318-25 / SNI
 * 2847:2019.
 *
 * Reproduces the EXACT mechanics of `phiMnBars` (beam.ts) — same εcu, fs clamp,
 * displaced-concrete correction, bisection, and φ ramp — but groups the bars
 * into depth *levels* and exposes per-level εs / fs / force so the
 * Section–Strain–Stress–Compatibility chart and the report table can render the
 * full derivation. Pure domain module: no React imports.
 *
...
import { buildBarLayout, type BarLayout, type LayoutBar } from "../../shared/bar-geometry"
import { barArea } from "../../shared/rebar"
import { beta1, EPS_CU, EPS_T_MIN } from "./rules"
import { avSProvided, vc, phiVnProvided } from "./beam"
